...
const embedConfig = require("./giveaway.embed.js");
...
client.once("ready", async () => {
  ...
  console.log(`\x1b[34m Version: ${pjson.version} \x1b[0m`);
  ...
  let statsChannel = guild.channels.cache.find((channel) =>
    channel.name.startsWith(config.channels.memberstats)
  );
  ...
  setInterval(() => {
    ...
    statsChannel
      .setName(`${config.channels.memberstats} ${memberCount}`)
      .catch(console.error);
  }, 300000);
...
client.giveawaysManager = new GiveawaysManager(client, {
  ...
  default: {
    botsCanWin: false,
    embedColor: config.giveaway.defaultColor,
...
client.giveawaysManager.on(
  ...
  (giveaway, member, reaction) => {
    ...
    const msgId = giveaway.messageId;
    const msgLink = `https://discord.com/channels/${guildId}/${channelId}/${msgId}`;

    const dmEmbed = new EmbedBuilder()
      .setTitle(embedConfig.join.title)
      .setDescription(
        embedConfig.join.description.replace("{msgLink}", msgLink)
      )
      .setColor(embedConfig.join.color)
      .setTimestamp()
      .setFooter({
        text: config.texte.footer,
        iconURL: config.bot.images.footerLogo,
      });
...
client.giveawaysManager.on("giveawayEnded", async (giveaway, winners) => {
  ...
  try {
    winners.forEach((member) => {
      const dmEmbed = new EmbedBuilder()
        .setTitle(embedConfig.end.title)
        .setDescription(
          embedConfig.end.description
            .replace("{winner}", member.user)
            .replace("{msgLink}", msgLink)
            .replace("{prize}", giveaway.prize)
        )
        .setColor(embedConfig.end.color)
        .setTimestamp()
        .setFooter({
          text: config.texte.footer,
          iconURL: config.bot.images.footerLogo,
    ...
    console.error("Fehler beim Senden der DMs an die Gewinner:", err);
  }
});
